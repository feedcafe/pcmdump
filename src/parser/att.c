...
/*********************************************************/
#define FRAME_SIZE		38	/* 3(0x9C 0x31 0x0F) + 35(audio data) */
#define BUF_SIZE		8192

uint8_t sbc_buf[FRAME_SIZE];	/* hold sbc encoded audio data */
uint8_t frame_num;
uint8_t bytes = 3;		/* the first 3-byte are sbc parameters */
uint8_t frame_start = 0;
sbc_t sbc;


/* sbc encoded data format
 * two ble packets form a audio frame, each ble packet starts with
 * a magic code, first packet magic code is 0x0bad, and the second
 * is 0xbabe
 * the first packet of the frame followed by frame number, here it is:
 *
 * #1 magic code + frame number + 17 bytes sbc audio data
 * 0x0b 0xad 0x00 ... ...
 *
 * #2 magic code + 18 bytes sbc audio data
 * 0xba 0xbe ... ...
 *
 * sbcinfo:
 * mSBC			0
...
static int ble_audio_decode(int fd, void *buf, int size)
{
	int framelen;
	size_t len;
	...
	/* decode audio data */
	framelen = sbc_decode(&sbc, buf, FRAME_SIZE, pcm_buf, BUF_SIZE, &len);
	printf("len=%d, frame len=%d\n", len, framelen);

	return write(fd, pcm_buf, len);
}

static int att_handle_ble_audio(struct frame *frm)
{
	uint8_t *ptr = frm->ptr;
	...
	/* ble audio start */
	if ((ptr[0] == 0x6A) && (ptr[2] == 0x67) &&(ptr[3] == 0x65)) {
		tm = *localtime(&frm->ts.tv_sec);
		sprintf(audio_file, "test-%d%02d%02d-%02d%02d%02d.pcm",
				tm.tm_year + 1900,
				tm.tm_mon + 1,
				tm.tm_mday,
				tm.tm_hour,
		...
		frm->audio_fd = open_pcm(audio_file);

		/* init sbc codec */
		sbc_init(&sbc, 0L);
		sbc.endian = SBC_LE;

		printf("skipping audio start packet\n");
		return -EAGAIN;
	}

	/* ble audio stop */
	if ((ptr[0] == 0x72) && (ptr[2] == 0x72) &&(ptr[3] == 0x72)) {
		close(frm->audio_fd);

		/* free memories used for decoding */
		sbc_finish(&sbc);

		printf("skipping audio stop packet\n");
		return -EAGAIN;
	}


	sbc_buf[0] = 0x9c;	/* sync word */
	sbc_buf[1] = 0x31;
	sbc_buf[2] = 0x0f;	/* bitpool */

	switch (ptr[0]) {
	case 0x0b:
		if (frame_start)
			printf("dropped incomplete frame %02x\n", ptr[2] - 1);

		if (ptr[1] == 0xad) {
			frame_start = 1;
			frame_num = ptr[2];
			memcpy(&sbc_buf[3], &ptr[3], 17);
			bytes = 20;
		}
		break;

	case 0xba:
		if ((frame_start == 1) && (ptr[1] == 0xbe)) {
			memcpy(&sbc_buf[20], &ptr[2], 18);
			bytes += 18;
		}
		break;
	...
	default:
		frame_start = 0;
		fprintf(stderr, "frame number is incorrect: %02x\n", ptr[1]);
		break;
	}

	if ((frame_start == 1) && (bytes == FRAME_SIZE)) {
#if 0
	return write(frm->audio_fd, sbc_buf, FRAME_SIZE);
#else
	ble_audio_decode(frm->audio_fd, sbc_buf, FRAME_SIZE);
#endif
		printf("sbc encoded data, frame number: %d(%02x)\n", frame_num, frame_num);
		frame_start = 0;
	}
	return 0;
}
...
static void att_handle_notify_dump(int level, struct frame *frm)
{
	...
	p_indent(level, frm);
	printf("handle 0x%4.4x, len: %d\n", handle, frm->len);

	/* dump ble audio data only */
	if (0x4a != handle)
		return;

	att_handle_ble_audio(frm);
}
